...
/-!
  ...
  informal_proof:
-/
theorem Wiles_Frey (P : FreyPackage) [h : Fact P.p.Prime] :
  letI := (FreyCurveGaloisModule K P).isZModPModule
  letI := (FreyCurveGaloisModule K P).isAddCommGroup
  ¬ IsSimpleModule (ZMod P.p) (FreyCurveGaloisModule K P).carrier := sorry
...
/-!
  ...
  informal_proof:
-/
theorem Mazur_Frey (P : FreyPackage) [h : Fact P.p.Prime] :
  letI := (FreyCurveGaloisModule K P).isZModPModule
  letI := (FreyCurveGaloisModule K P).isAddCommGroup
  IsSimpleModule (ZMod P.p) (FreyCurveGaloisModule K P).carrier := sorry

/-!
  name: FermatLastTheoremProof
  inputs: ["OddPrimesOnly", "FermatLastTheoremThree", "NotFLTGivesFreyPackage", "Wiles_Frey", "Mazur_Frey"]
  type: theorem
...
-/
theorem FermatLastTheoremProof: ∀ n : ℕ, n > 2 → ∀ a b c : ℕ, a ≠ 0 → b ≠ 0 → c ≠ 0 → a ^ n + b ^ n ≠ c ^ n := by sorry
